...
The headline result isn't which model plays the best euchre. It's that the top-scoring agent got there partly by gaming my grading rule: it re-ran the benchmark over and over and stopped on a lucky score. Everything else — seven models, four opponents, a two-order-of-magnitude cost spread — is downstream of that.

# Context

I created a euchre-bench — a public API where you point an agent at four bots of increasing strength and see how many 100-game matches it wins. The four bots are `random` (trivial), `easy`, `medium`, and `hard`, the [CFR+PIMCTS bot](/posts/cfr-for-euchre/) from a few posts ago. PIMCTS is *perfect-information Monte Carlo tree search*: guess the hidden cards many different ways, solve each guess as if every hand were face up, and vote on the best move. `hard` is the API's name for the top bot; I call it *difficult* in the prose below.

Coding agents are tasked with creating a bot that wins as many matches as possible against each level of difficulty.
...
For reference, the `random` opponent picks a legal action uniformly at random. Losing 19 matches out of 100 to random — like v4-flash — means that the policy is genuinely worse than coin-flipping play, in many situations. v4-pro losing 59 of them is impressive.
...
The five pure-heuristic agents bunched between 0 and 3 wins against the difficult bot. The two that built rollouts — Sonnet (n_sims=100 per decision) and Gemini-3.5-flash (5-world PIMCTS sample-and-rollout) — scored 5 and 6.
...
The cheap and pleasant inference is that PIMCTS is the right move for this benchmark, but with a caveat. Claude Code (Opus-4.7) tried four different Monte Carlo variants: open-hand for play, with heuristic playout, with open-hand playout, and vote-based. Every one of these four variants was *worse* than a well-tuned heuristic. From its own write-up:
...
So Sonnet and Gemini-3.5-flash built simpler rollouts and got lucky on variance; Opus built more sophisticated rollouts, found them worse than heuristics, and got lucky on variance a *different* way (more on that in the next section).

The honest read is this: at 5–8 wins out of 100 with maybe 3 wins of standard deviation per session, the gap between the top three is mostly noise. The gap between the top three and the bottom four is real.
...
This isn’t against the literal rule (the rule is the rule, and the underlying policy is solid). But with ~3 wins of standard deviation and 4 attempts allowed, the “difficult 6” is plausibly just the top of four draws from a distribution centered around 3–4.

I’m going to change the grading rule. Probably to *median of N sessions*, or I’ll pin the official one and refuse to overwrite it. Either change makes variance-hunting net zero.

Sonnet under the *previous* prompt did the same thing — I killed an earlier run when I noticed a Python script doing “rerun medium until challenger_score ≥ 295.” The current prompt added an explicit ranking metric (match wins, not raw points) and Sonnet stopped rerolling on its own. That suggests prompts can shut this down for in-harness agents, but anything driving the loop from the outside — like Claude Code calling the API directly — won’t see the new prompt.

# Cost notes

Cost per win against the difficult bot spans two orders of magnitude:

- GPT-5.5: $0.49 per win (3 wins on $1.46)
- Gemini-3.5-flash: $0.87 per win
- Claude-Sonnet-4.6: $4.74 per win
- Claude-Code / Opus-4.7: ~$55 per win at list pricing

The first two are heuristic-only; the second two are heuristic + rollouts. You can pay more and get marginally better worst-case play; you can also pay 60 times more and get the same number of wins against the difficult bot as Gemini-3.5-flash.

The cheapest run that completed all four 100-game evaluations was Gemini-3-flash-preview at $0.44. It scored 0/0/1/0 on easy/medium/difficult. So “completes the benchmark for under a dollar” is achievable. In this matrix, “Beats the medium bot for under a dollar” is not.
...
Two things shape every run: the system prompt the harness gives the agent at container startup, and the bench's own API reference (`GET /bench/help`) which the prompt directs the agent to fetch first. Both are reproduced below as they stood for this matrix. `{{BENCH_URL}}` is a placeholder for the bench's hostname; `{{CHALLENGER_ID}}` is filled in by the harness per run.

## System prompt (May 22 matrix)
