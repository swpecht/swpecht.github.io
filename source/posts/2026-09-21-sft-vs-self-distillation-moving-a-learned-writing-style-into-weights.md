...
- **SFT also damages the model, and SDFT doesn't.** On 20 general prompts (arithmetic, counting letters, reversing a word), Inkling-Small went from 100% to 70% after SFT and stayed at 100% after every SDFT variant. SFT also produced a few broken outputs with raw chat-template tokens in them; SDFT produced none.
...
So it is a trade-off, and now I have numbers for it. SFT gives the full style and costs 20–30 points on general prompts. SDFT with a rich teacher gives a bit more than half of the stylistic movement and costs nothing I could measure.
...
- *Retention.* 20 short prompts that have nothing to do with the style task, each with a code check: a two-step word problem, count the r's in "strawberry refrigerator", write "consolidation" backwards, an acrostic, a sentence with no letter e, and so on. I also checked whether the newsletter format leaked into these answers. It never did.
- *Faithfulness.* A judge lists factual claims in the draft that the brief doesn't support.

# Style: SFT wins

| Model | Variant | Stylometric distance | Judged closer than untrained |
|---|---|---|---|
| Qwen3-8B | untrained | 1.40 | |
| Qwen3-8B | SFT | **0.70** | 30/30, 30/30 |
| Qwen3-8B | SDFT | 1.15 | 29/30, 30/30 |
| Inkling-Small | untrained | 1.16 | |
| Inkling-Small | SFT | **0.77** | 30/30, 30/30 |
| Inkling-Small | SDFT | 1.10 | 21/30, 22/30 |
...
| Inkling-Small | Retention, 20 prompts | Broken outputs, of 30 test drafts |
|---|---|---|
| untrained | 100% | 0 |
| SFT | **70%** | 0 |
| SDFT | 100% | 0 |
| SDFT, rich teacher | 100% | 0 |
| SFT in three rounds, after each | **85 / 80 / 80%** | 6 / 0 / 2 |
| SDFT in three rounds, after each | 100 / 100 / 100% | 0 / 0 / 0 |
...
Qwen3-8B doesn't show this, because with thinking disabled the untrained model only passes 35% of these prompts, and everything else lands between 20% and 45% with no pattern. The set is the right difficulty for Inkling-Small and too hard for a non-thinking 8B.
...
The damage from SFT did not accumulate. On Inkling-Small the retention score was 85% after the first round and 80% after the second and third. Three rounds is too short a sequence to conclude much from that.
...
- SDFT for not breaking the model. On Inkling-Small every SDFT model passed all 20 retention prompts and produced no broken outputs.
...
- The retention set on Qwen3-8B. Too hard for the base model, so it shows nothing there. One set does not fit two models.
...
- 20 retention prompts is still a small check, and it isn't a benchmark anyone else uses.
