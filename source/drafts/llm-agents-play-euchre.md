...
I ran seven models through it: sonnet-4.6, opus-4.7 (via Claude Code), gpt-5.5, gemini-3-flash-preview, gemini-3.5-flash, deepseek-v4-flash, and deepseek-v4-pro. Each one gets a fresh sandbox, the same system prompt, and a few hours to do whatever it wants. The harness and the raw event streams are on [github](https://github.com/swpecht/euchre-bench); you can browse the [trajectories](/trajectories/) directly.
...
Things I want to change about the bench:

- **Grading**: take the median of N=5 sessions. Removes the variance-hunting incentive without preventing iteration.
- **Hide source-code recon**: not realistic. The blog post about the easy bot is the most useful single page on the internet for this benchmark, and gemini-3.5-flash went straight from reading it to building 5-world PIMCTS. That's exactly the loop I wanted to encourage.
- **Add a `random_legal_action_validator` endpoint**: the only consistent agent error was missed legal-action lists. Giving them a way to verify a candidate action would let me see whether the policy or the encoding is the bottleneck.

You can browse the full agent event streams [here](/trajectories/) — each one is the entire trajectory of tool calls, assistant text, and intermediate outputs. The deepseek-v4-pro one is particularly fun: 145 single-game probes against the random bot in a row, just to map the action space.
