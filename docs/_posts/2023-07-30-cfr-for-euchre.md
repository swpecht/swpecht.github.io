...
> Decision points that are similar in terms of what information has been revealed (in poker, the player’s cards and revealed board cards) are bucketed together and treated identically. For example, a 10-high straight and a 9-high straight are distinct hands but are nevertheless strategically similar. Pluribus may bucket these hands together and treat them identically, thereby reducing the number of distinct situations for which it needs to determine a strategy.
 ...
 I did not take this approach for two reasons: 1) coding up game specific information state abstractions takes time 2) even a training time of 8 days is too long for me to experiment with. I don't really know what I'm doing and need to get faster feedback on if things are working.
...
My hypothesis is that these later moves are less important and a simpler evaluation function may be good enough.

With this hypothesis, I only used CFR for the pre-play phases of euchre: telling the dealer whether to pickup or pass, calling a suit or passing, and discarding a card. Instead of using the true regrets for CFR, I use the evaluation from an [open hand solver](/project-log/2023/07/15/pimcts-for-euchre).

With this change, I only need to store 1.1m information states for games where the `Js` is the face up card. And in practice I store about 40% of this many as external sampling guides towards more realistic moves to explore. See [estimating number of information state](#estimating-number-of-information-states) for more details.

# Training and performance versus PIMCTS

The pre-play CFR agent was trained for 20m deals with a `Js` as the face up card. The training took 50 hours on a Hetzner AX41-NVMe (AMD Ryzen 5 3600 with 64 GB DDR4) on a single thread . The resulting weights serialize to 63MB ([json download](https://fewworddotrick.blog.nyc3.cdn.digitaloceanspaces.com/infostates.open-hand-20m.json)).
...
To compare performance to the PIMCTS algorithm, I had the CFR agent and the PIMCTS agent both play as dealers against the PIMCTS agent. For the CFR agent, I used the CFR results to choose move for all pre-play actions and then used PIMCTS for the play phase. All PIMCTS agents used 50 worlds for move evaluation.
...
It also looks like the "usually pickup" and "rarely pickup" bucket would disappear with enough time for the CFR training to converge:
...
A spot check of the hand seem to make intuitive sense:
...
But there are some unexpected recommendations. For example, `JcQcJhAh9d` is a never pickup hand. But if I were playing, I would be tempted to pickup. The dealer would have the 2 highest cards (`Js` and `Jc`) and an off suit ace. More research is needed to understand if this is an incorrect play caused by the lack of strategy fusion for the card play portion of the game or the correct strategy.

While it's a usually a good idea to take the bower, sometimes passing on the bower is the right choice.

# Future work

The analysis of the pre-play CFR agent still has a number of shortcomings:
...
* It doesn't fully solve euchre. We still depend on PIMCTS for the play phase of the game
* We don't have a theoretically rigorous way to gauge it's performance
...
* The CFR portion of evaluation could be extended to cover more of the game (e.g. the first trick of play or all of the game)
...
# Estimating number of information states

There are 33k possible information states for a deal of cards if we constrain the `Js` to be the face up card. 

$$\binom{23}{5}=33649$$
      
There are 32 possible outcomes for the bidding phase: any of the 4 players could pickup or pass (8) and any of the 4 players could call a suit (16)

All together there are **1.1m pre-play information states** for euchre. This can easily be fit in memory.
...
I originally attempted to train the CFR agent using the evaluation of moves from a PIMCTS agent with 50 worlds rather than just the single evaluation from the open hand solver. My hypothesis was that this would make each iteration of CFR "more valuable" by returning a better approximation of the average regret.
