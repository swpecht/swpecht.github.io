...
class DeepCFRSolver(policy.Policy):
    ...
    def _reinitialize_policy_network(self):
        ...
        with tf.device(self._train_device):
            self._policy_network = PolicyNetwork(
                self._embedding_size, self._policy_network_layers, self._num_actions
            )
    ...
    def _reinitialize_advantage_network(self, player):
        ...
        with tf.device(self._train_device):
            self._adv_networks_train[player] = AdvantageNetwork(
                self._embedding_size, self._advantage_network_layers, self._num_actions
            )
    ...
    def _add_to_strategy_memory(
        self, info_state, iteration, strategy_action_probs, legal_actions_mask
    ):
    ...
    def _serialize_advantage_memory(
    ...
    def _traverse_game_tree(self, state, player):
        ...
        if state.is_terminal():
            ...
            self._advantage_memories[player].add(
                self._serialize_advantage_memory(
    ...
    def _get_matched_regrets(self, info_state, legal_actions_mask, player):
        ...
        advs = self._adv_networks[player](
            (tf.expand_dims(info_state, axis=0), legal_actions_mask), training=False
        )[0]
    ...
    def action_probabilities(self, state):
        ...
        probs = self._policy_network(
            (info_state_vector, legal_actions_mask), training=False
        )
        probs = probs.numpy()
