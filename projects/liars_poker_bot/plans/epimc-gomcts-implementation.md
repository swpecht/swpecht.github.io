...
| OH-4 | 2026-06-10 | Calibration: PIMCTS-50 subject vs random table (t1-5, 300/trick) | OH_SUBJECT=pimcts knob | pimcts: +1.40/+1.13/+0.92/+0.68/+0.37, pooled **+0.90**. Our t1-5 model: +1.24/+0.98/+0.64/+0.28/−0.04, pooled +0.62 | (a) high-trick random tables compress everyone's margin (pimcts decays too); (b) the model trails pimcts by ~0.3 off-distribution while matching it on-distribution — imitation distribution shift: the LM never saw post-random-move contexts. Candidate fix if it matters: add a pimcts-vs-random data band (Euchre exploiter pattern). The headline target (beat pimcts ON pimcts tables) is unaffected |
| OH-5 | 2026-06-10 | Zero-shot transfer test: t1-5 model at t6-8 vs pimcts | 120 games/trick | t6 −0.49, t7 −0.21, t8 −0.19; pooled **−0.30 ± 0.17** | substantial cross-trick transfer (random-level would be far worse). → cut high-trick collection quotas (t6-7: 12k→4k, t8-10: 9k→1.5k games; original quotas were 15h/50h of pimcts CPU) and rely on fine-tuning |
| OH-6 | 2026-06-10 | Full t1-10 model v1 (735k examples: 270k/330k/90k/45k per band) | 6 epochs; eval 300/trick vs PIMCTS-50, 600/trick vs random, greedy-LM | vs pimcts: t1 +0.19, t2 −0.38, t3 +0.11, t4 −0.25, t5 −0.24, t6 −0.39, t7 −0.70, t8 −0.46, t9 −0.35, t10 −0.18; pooled **−0.265 ± 0.06**. vs random pooled +0.27 (t6-10 ≈ 0 — same off-distribution decay as Euchre) | parity at t1-3, behind at t4-10 — performance tracks per-band data volume. User's open-hand-solver optimizations landed mid-collection (~6-10× faster pimcts data) → supplement bands launched: t6-7 +12k, t8-10 +6k games |
| OH-7 | 2026-06-11 | Full model v2 (1.19M examples; high-trick data ×4.3) | 6 epochs; 300/trick vs PIMCTS-50 | t1 +0.24, t2 −0.47, t3 +0.15, t4 +0.08, t5 −0.10, t6 −0.44, t7 −0.41, t8 −0.31, t9 −0.31, t10 −0.27; pooled **−0.184** (v1: −0.265) | high-trick data scaling works (t4 flipped +, t7/t8 +0.3) but slope is diminishing. t2 anomaly persists across every model (−0.38..−0.47 with 90k games of data; win% >50% yet mean negative → asymmetric bid-miss losses) — worth a focused investigation. v3 overnight: one more doubling of t6-10 data |
| OH-8 | 2026-06-11 | v3 pipeline halted by user mid-collection | t6-7 +24k band was ~80% done (cache unwritten — band caches write atomically at completion, so the partial work is discarded) | — | **current best Oh Hell model: `bootstrap_v2.safetensors`** (greedy-LM, scalar head, paper config). Pooled −0.184 vs PIMCTS-50: parity t1-4, behind t6-10. To resume the data-scaling path later: `./scripts/oh-hell-v3-overnight.sh \| kestrel-tail oh-hell-v3` — idempotent, skips finished bands, retrains v3 on all caches and evals. Open lead worth investigating first: the t2 anomaly (every model ~−0.4 at t2 despite >50% win rate → asymmetric bid-miss losses, not a data-volume issue) |
