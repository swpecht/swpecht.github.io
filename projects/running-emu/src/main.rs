...
fn new_world() -> World {
    World { costs: [Some(0); 100], start: Point {x: 0, y: 0}, goal: Point{x: 9, y: 9} }
}
    ...
    for y in 0..10 {
        for x in 0..10 {
            ...
            if p == world.start {
                ...
                print!("{}", world.costs[y * 10 + x].unwrap_or(-1))
            }
            print!("\t")            
        }
        println!("");
...
fn get_distance_matrix(world: World) -> [Option<i8>; 100] {
    let mut dmatrix: [Option<i8>; 100] = [None; 100];
    let mut queue: VecDeque<Point> = VecDeque::new();

    queue.push_back(world.start);
    let start = world.start;
    dmatrix[start.y * 10 + start.x] = Some(0);
    ...
    while !queue.is_empty() {
        ...
        let d = dmatrix[node.y * 10 + node.x].unwrap();
        for n in neighors {
            if dmatrix[n.y * 10 + n.x].is_none() {      
                dmatrix[n.y * 10 + n.x] = Some(d+1);
                queue.push_back(n);
            }
        }
