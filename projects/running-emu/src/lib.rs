...
mod tests {
    ...
    use crate::{Point, create_map, get_path};
    ...
    fn find_path_no_cost() {
        let path = get_path(Point{x: 0, y: 0}, Point{x: 2, y: 2}, &vec![vec![Some(0); 3]; 3]);
        assert_eq!(path.unwrap(), vec![Point { x: 0, y: 0 }, Point { x: 0, y: 1 }, Point { x: 0, y: 2 }, Point { x: 1, y: 2 }, Point { x: 2, y: 2 }])
    }

    #[test]
    fn find_path_cost() {
        let path = get_path(Point{x: 0, y: 0}, Point{x: 2, y: 2}, &vec![vec![Some(0); 3], vec![Some(10), Some(10), Some(0)], vec![Some(0); 3]]);
        assert_eq!(path.unwrap(), vec![Point { x: 0, y: 0 }, Point { x: 1, y: 0 }, Point { x: 2, y: 0 }, Point { x: 2, y: 1 }, Point { x: 2, y: 2 }])
    }
}
...
pub fn get_path_from_distances(start: Point, end: Point, distance_matrix: &Vec<Vec<Option<i32>>>) -> Vec<Point> {
    ...
    while path.last().unwrap().clone() != start {
        ...
        let (_, min) = neighbors.iter().filter(|p| !distance_matrix[p.y][p.x].is_none()).enumerate().min_by(|a, b| distance_matrix[a.1.y][a.1.x].cmp(&distance_matrix[b.1.y][b.1.x])).unwrap();
        path.push(*min);
    }
...
pub fn attacker_system_update(world: &mut World, agent: &mut AttackerAgent) -> bool {
    ...
    if agent.queue.is_empty() {
        ...
        agent.queue.push(cur_loc, Reverse(0));
        agent.update_cost(cur_loc, 0);
    }
    ...
    if cur_loc != agent.next_target.unwrap() {
        ...
        for p in path {
            ...
            match e {
                Some(e) => world.add_component_to_entity(e, BackgroundHighlight(Color::Blue)),
                _ => {}
            }            
        }
    }
